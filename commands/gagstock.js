...
module.exports = {
    ...
    async execute(senderId, args, pageAccessToken) {
        ...
        if (action === "off") { const session = activeSessions.get(senderId); if (session) { clearTimeout(session.timeout); activeSessions.delete(senderId); lastSentCache.delete(senderId); logger.warn(`Stopped tracking for session: ${senderId}`); return await sendMessage(senderId, { text: "🛑 *Tracking Stopped*\nYou will no longer receive stock updates." }, pageAccessToken); } return await sendMessage(senderId, { text: "⚠️ *No Active Session*\nUse `!gagstock on` to start tracking." }, pageAccessToken); }
        if (action !== "on") { const helpMessage = `❓*Invalid Usage*\n\nPlease use one of these formats:\n› \`!gagstock on\`\n› \`!gagstock on [item]\`\n› \`!gagstock off\``; return await sendMessage(senderId, { text: helpMessage }, pageAccessToken); }
        if (activeSessions.has(senderId)) { return await sendMessage(senderId, { text: "📡 *Already Tracking*\nA session is already active for you. Use `!gagstock off` to stop it first." }, pageAccessToken); }

        const initialMessage = filters.length > 0 ? `🚀 *Tracking Started with Filters!*\nYou'll be notified for changes to: *${filters.join(", ")}*` : "🚀 *Tracking Started!*\nYou will now receive live updates for Grow A Garden stock.";
        await sendMessage(senderId, { text: initialMessage }, pageAccessToken);
        logger.success(`Started new tracking session for ${senderId}`);
        ...
        const fetchAndNotify = async (isInitialCall = false) => {
            ...
            try {
                ...
                // We must define 'backup' before we can use it.
                const backup = stockRes.data.data;
                const stockData = {};
                
                spinner.text = `Processing stock...`;
                ...
                if (!isInitialCall && lastSentCache.get(senderId) === currentKey) {
                ...
                if (message.startsWith(" Mothing Found")) {
                    await sendMessage(senderId, { text: " Mothing Found No items matched your filter. Tracker is still running in the background." }, pageAccessToken);
                    return false;
                }
                ...
                if (spinner) spinner.fail(`Failed to process data for ${senderId}.`);
                logger.error(error);

                if (isInitialCall) {
                    await sendMessage(senderId, { text: "💥 Oops! I couldn't fetch the stock data. The API might be down. I've stopped the tracker, please try again in a moment by typing `!gagstock on`." }, pageAccessToken);
                    clearTimeout(activeSessions.get(senderId)?.timeout);
        ...
        const scheduleNextRun = () => {
            ...
            const wait = Math.max(nextTime - getPHTime(), 1000);
            logger.info(`Scheduling next check for ${senderId} in ${Math.round(wait / 1000)}s.`);
            const timeoutId = setTimeout(async () => {
                const wasSuccessful = await fetchAndNotify();
                // Only schedule the next run if the fetch was successful
                // to prevent spamming on a failing API.
                if(wasSuccessful) {
                    scheduleNextRun();
                } else {
                    logger.warn("Fetch failed. Pausing schedule for this session. It will retry on the next command.")
                }
            }, wait);
            activeSessions.set(senderId, { timeout: timeoutId, filters });
        };
